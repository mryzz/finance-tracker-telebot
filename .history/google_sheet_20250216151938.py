...
class GoogleSheets:
    ...
    def check_balance(self):
        ...
        try: 
            combined = "Monthly budget | Monthly expense | Monthly left "
            sheet = self.service.spreadsheets()
            range_names = ["H:H", "J:J", "K:K", "L:L"]
            result = (
                sheet
                .values()
            ...
            month_list = result["valueRanges"][1]['values'][1:]
            ...
            for i in range(len(index_list)):
                if int(index_list[i][0]) == current_month:
                    return f"Current month: {month_list[i][0]}   Budget: {budget_list[i][0]}   Expense: {expense_list[i][0]}   Net cash: {net_cash_list[i][0]}"
            for i in range(length):
                combined += f"{month_list[i][0]}   {budget_list[i][0]}   {expense_list[i][0]}   {net_cash_list[i][0]}\n"
            return combined  
