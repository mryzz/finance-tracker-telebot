...
class GoogleSheets:
    ...
    def check_bsf_balance(self):
        """Reads BSF balance from a Google Sheet."""
        result = self.get_data("'Compute (2025)'!H:I")
        return f"BSF balance: {result[1][1]}\n"
    ...
    def append_values(self, _values):
        ...
        try:
            ...
            values = [[datetimenow, _values[0], _values[1].lower(), _values[2].lower(), today.month]]
            body = {"values": values}
            result = (
                sheet
                .values()
            ...
            self.recent_entry_cells = result['updates']['updatedRange']
            return result
    ...
    def delete_recent_entry(self):
        ...
        sheet = self.service.spreadsheets()
        try:
            last_row = None  # Initialize last row variable
            delete_range = None  # Initialize range variable

            # Fetch data if recent entry is unknown
            if self.recent_entry_cells is None:
                values = self.get_data("'Data (2025)'!A:E")  # Fetch all rows
                
                if not values or len(values) == 1:  # Check if data is empty or only headers
                    print("No data available to delete.")
                    return None

                last_row = len(values)  # Assuming no empty rows in between
                delete_range = f"'Data (2025)'!A{last_row}:E{last_row}"  # Target last row
            else:
                delete_range = self.recent_entry_cells  # Use stored range
                self.recent_entry_cells = None  # Reset stored range
            ...
            print("No valid range found to delete.")
            return None
...
# print(google_sheets.check_bsf_balance())
